  ...
  DO jz = 1,nz
    DO jy = 1,ny
      DO jx = 1,nx
        ...
        WRITE(8,184) x(jx)*OutputDistanceScale,y(jy)*OutputDistanceScale,(spsurf10(ns,jx,jy,jz),ns = 1,nsurf+nsurf_sec)
      END DO
    END DO
  END DO
  CLOSE(UNIT=8,STATUS='keep')
...
!fp! if_onproc({#expr# spex10(nex,jx,jy,jz) #});
        AqueousToBulk = ro(jx,jy,jz)*satliq(jx,jy,jz)*por(jx,jy,jz)
        WRITE(8,184) x(jx)*OutputDistanceScale,z(jz)*OutputDistanceScale,(spex10(nex+nexchange,jx,jy,jz),nex = 1,nexch_sec)
!fp! end_onproc();
      END DO
    END DO
  END DO
